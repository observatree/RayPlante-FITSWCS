...
/**
 *   This class provides support for the Mercator projection (MER) 
 ...
 * @version 2.4
 *---------------------------------------------------------------------------*/
public class MERProjection extends Projection {
     ...
     */
    public MERProjection() { init(0, new double[1]); }

    /**
     * Create an MERProjection object
     * @param r0 sphere radius
     */
    public MERProjection(double r0) { init(r0, new double[1]); }
     ...
     */
    public MERProjection(double r0, double[] p) 
    {
	init(r0, p);
    }
	...
	if (theta <= -90.0 || theta >= 90.0) 
	    throw new PixelBeyondProjectionException("MER: abs(theta=" + theta +
						     ") > 90.0");
	out[0] = w[0]*phi;                           
	out[1] = r0*Math.log(TrigD.tan((90.0+theta)/2.0));
	return out;
    }
	...
	out[0] = w[1]*x;
	out[1] = 2.0*TrigD.atan(Math.exp(y/r0)) - 90.0;
	return out;
    }
