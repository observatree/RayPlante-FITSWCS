...
/**
 *   This class provides support for the zenithal/azimuthal equal area 
 *   projection (ZEA) used by the FITS "World Coordinate System" (WCS) 
 *   convention. <p>
 *
 ...
 * @version 2.4
 *---------------------------------------------------------------------------*/
public class ZEAProjection extends Projection {
     ...
     */
    public ZEAProjection() { init(0, new double[1]); }

    /**
     * Create an ZEAProjection object
     * @param r0 sphere radius
     */
    public ZEAProjection(double r0) { init(r0, new double[1]); }
     ...
     */
    public ZEAProjection(double r0, double[] p) 
    {
	init(r0, p);
    }
	...
	r =  w[0]*TrigD.sin((90.0 - theta)/2.0);
	...
	if (Math.abs(r-w[0]) < tol) {
	    out[1] = -90.0;
	} else {
	    out[1] = 90.0 - 2.0*TrigD.asin(r*w[1]);
	}
