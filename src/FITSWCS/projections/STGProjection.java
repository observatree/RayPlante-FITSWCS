...
/**
 *   This class provides support for the stereographic projection (STG) used 
 ...
 * @version 2.4
 *---------------------------------------------------------------------------*/
public class STGProjection extends Projection {

    /**
     * Create an STGProjection object
     */
    public STGProjection() { init(0, new double[1]); }

    /**
     * Create an STGProjection object
     * @param r0 sphere radius
     */
    public STGProjection(double r0) { init(r0, new double[1]); }
     ...
     */
    public STGProjection(double r0, double[] p) 
    {
	if (p != null) {
	...
	s = 1.0 + TrigD.sin(theta);
	if (s == 0.0) 
	    throw new PixelBeyondProjectionException("STG: theta = " + theta);
	...
	out[1] = 90.0 - 2.0*TrigD.atan(r*w[1]);
