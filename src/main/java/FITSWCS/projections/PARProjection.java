...
/**
 *   This class provides support for the parabolic projection (PAR) 
 ...
 * @version 2.4
 *---------------------------------------------------------------------------*/
public class PARProjection extends Projection {
     ...
     */
    public PARProjection() { init(0, new double[1]); }

    /**
     * Create an PARProjection object
     * @param r0 sphere radius
     */
    public PARProjection(double r0) { init(r0, new double[1]); }
     ...
     */
    public PARProjection(double r0, double[] p) 
    {
	init(r0, p);
    }
	...
	s = TrigD.sin(theta/3.0);
	out[0] = w[0]*phi*(1.0 - 4.0*s*s);
	out[1] = w[2]*s;
	return out;
    }
	...
	if (s > 1.0 || s < -1.0) 
	    throw new PixelBeyondProjectionException(
	        "PAR: Solution not defined for y: " + y);
	...
	if (t == 0.0) {
		...
		throw new PixelBeyondProjectionException(
		    "PAR: Solution not defined for x,y: " + x + ", " + y);
	    }
	} else {
	    out[0] = w[1]*x/t;
	}

	out[1] = 3.0*TrigD.asin(s);
	return out;
    }
