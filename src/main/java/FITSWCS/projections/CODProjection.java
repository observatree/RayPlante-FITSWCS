 ...
 * @version 2.4
 *---------------------------------------------------------------------------*/
public class CODProjection extends Projection {
     ...
     *               mu + lambda = 0;
     */
    public CODProjection(double sigma, double delta) 
     ...
     *               mu + lambda = 0;
     */
    public CODProjection(double r0, double sigma, double delta) 
	throws BadProjectionParameterException
    {
	double[] use = { 0, sigma, delta };
	init(r0, use);
     ...
     *               p[1] + p[2] = 0;
     */
    public CODProjection(double r0, double[] p) 
     ...
     */
    public CODProjection() { r0 = R2D; }

    /**
     * sets up intermediate data
     * <pre>
     *   r0     reset to 180/pi if 0.
     *   w[0]   C = r0*sin(sigma)*sin(delta)/delta
     *   w[1]   1/C                               
     *   w[2]   Y0 = delta*cot(delta)*cot(sigma)  
     *   w[3]   Y0 + sigma                        
	...
	if (p[2] == 0.0) {
	    w[0] = r0*TrigD.sin(p[1])*D2R;
	} else {
	    w[0] = r0*TrigD.sin(p[1])*TrigD.sin(p[2])/p[2];
	}
	if (w[0] == 0.0) throw new 
	    BadProjectionParameterException("Bad value for sigma: " + p[1]);

	w[1] = 1.0/w[0];
	w[2] = r0*TrigD.cos(p[2])*TrigD.cos(p[1])/w[0];
	w[3] = w[2] + p[1];
    }	
